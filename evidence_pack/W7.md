...
This estimate is intentionally small because the app is a low-traffic
hackathon deployment. Most of the application path is serverless or
usage-driven: S3 and CloudFront serve a static frontend, API Gateway and
Lambda run only when demo requests arrive, DynamoDB is on-demand, and
Bedrock/Knowledge Base cost scales with the number of prompts,
embeddings, and retrieval calls. At this scale, several services remain
inside or close to AWS Free Tier thresholds, so the live AWS billing
pages can legitimately show **USD 0.00** for some views even while the
deployed resources are working.
...
If Cost Explorer shows **USD 0.00** or the tag filter has no matching
cost rows, that does not mean the StudyBot stack is missing. It means
the measured billable usage is tiny, still covered by Free Tier, still
inside AWS billing-data delay, or not yet allocated to newly activated
cost allocation tags. Cost allocation tags are not retroactive and can
take up to 24 hours to appear in Cost Explorer filters after activation,
so this pack pairs billing screenshots with Resource Groups Tagging API
evidence for the live tagged resources.
